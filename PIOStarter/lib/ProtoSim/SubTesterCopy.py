...
# --- 2. Global State ---
motors = [1500] * 8  # Unified value (updates immediately on screen)
active_motors = [1500] * 8  # Track what was last SENT to the Pico
selected_idx = 0
group_mode = False
live_mode = False
sine_mode = False
...
with term.fullscreen(), term.cbreak(), term.hidden_cursor():
    ...
    while True:
        ...
        with term.location(0, 0):
            ...
            print(f"Port: {detected_port} | {term.bold}R{term.normal}: Ramp | {term.bold}L{term.normal}: Live | {term.bold}ENTER{term.normal}: Send | {term.bold}S{term.normal}: Stop")
            print("-" * term.width)
        ...
        for i in range(8):
            with term.location(2, 4 + i):
                ...
                # Bar reflects the value currently on screen
                ...
                # Highlight value in Yellow if it hasn't been "sent" yet
                val_color = term.bold
        ...
        # Ramp Logic (Ramps toward 'motors' list)
        elif key.lower() == 'r':
