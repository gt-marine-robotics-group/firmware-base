...
#                                 sensors["temp"] = env.readings.temperature
#                             elif stype == "amperage":
#                                 sensors["amp"] = env.readings.amperage
#                             elif stype == "accel":
#                                 sensors["accel"]["x"] = env.readings.accel.x
#                                 sensors["accel"]["y"] = env.readings.accel.y
#                                 sensors["accel"]["z"] = env.readings.accel.z
#                     except: pass
#             time.sleep(0.01)
                ...
                while b'\x00' in buffer:
                    ...
                    try:
                        ...
                        if ptype == "status":
                            ...
                            # Updated to capture BOTH ID and Content
                            pico_status["debug_id"] = env.debug.id
                            pico_status["debug"] = env.debug.content
                        elif ptype == "readings":
                            stype = env.readings.WhichOneof("data")
                            if stype == "temperature":
                                ...
                                sensors["accel"] = {
                                    "x": env.readings.accel.x,
                                    "y": env.readings.accel.y,
                                    "z": env.readings.accel.z
                                }
                    except: pass
            time.sleep(0.01)
        except Exception: break

# Initialize the new state key
pico_status["debug_id"] = 0
...
with term.fullscreen(), term.cbreak(), term.hidden_cursor():
    ...
    while True:
        ...
        with term.location(0, 0):
            ...
            print(f"Port: {detected_port} | {term.bold}F{term.normal}: Toggle Step | {term.bold}T{term.normal}: Sine | {term.bold}Z{term.normal}: Zero | {term.bold}ENTER{term.normal}: Send")
            print("-" * term.width)

        # Motor Column (Left Side)
        for i in range(8):
            with term.location(2, 4 + i):
                ...
                # Clear the line slightly more to prevent overlapping
                print(f"{prefix}{motor_label}: [{term.green}{bar_str}{term.normal}] {val_color}{motors[i]:.2f}{term.normal}      ")

        # Sensor Side Panel (Pushed further Right to X=60)
        # This keeps it clear of the bars and labels
        with term.location(60, 4):
            print(term.bold + "SENSOR TELEMETRY" + term.normal)
        with term.location(60, 5):
            print(f"Temp: {term.yellow}{sensors['temp']:.1f}°C{term.normal}   ")
        with term.location(60, 6):
            print(f"Amp:  {term.red}{sensors['amp']:.2f} A{term.normal}    ")
        with term.location(60, 7):
            print(f"Accel X: {sensors['accel']['x']:.2f} g  ")
        with term.location(60, 8):
            print(f"Accel Y: {sensors['accel']['y']:.2f} g  ")
        with term.location(60, 9):
            print(f"Accel Z: {sensors['accel']['z']:.2f} g  ")
        ...
        with term.location(0, 14):
            # 1. Clear the lines before drawing to prevent horizontal artifacts
            ...
            # Line 15: Status Row
            print(term.clear_eol + f"MODE: {mode_str} | SINE: {sine_str} | STEP: {step_display} | GROUP: {term.cyan}{group_mode}{term.normal}")
            
            # Line 16: Debug Row
            # Added debug_id and term.clear_eol to wipe trailing characters
            debug_info = f"[{pico_status['debug_id']}] {pico_status['debug']}"
            print(term.clear_eol + f"E-Stop: {estop_color}{pico_status['estop']}{term.normal} | LAST DEBUG: {term.yellow}{debug_info}{term.normal}")
        ...
        if key.lower() == 'q': break
            ...
            if step_mode == "FINE":
                step_mode, current_step = "COARSE", 0.25
            else:
                step_mode, current_step = "FINE", 0.05
        elif key.lower() == 'z':
        ...
        # Sinusoidal Sweep Logic (Full -1 to 1)
        if sine_mode:
            sine_offset += 0.1
